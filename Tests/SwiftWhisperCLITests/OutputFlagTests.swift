...
struct OutputFlagTests {
    ...
    func tildeExpansion() throws {
        let unique = "swiftwhisper-output-\(UUID().uuidString).txt"
    ...
    static func tempAudio() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("swiftwhisper-output-flag-\(UUID().uuidString).wav")
    ...
    static func tempPath() -> String {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("swiftwhisper-output-\(UUID().uuidString).txt")
            .path
    }
}
