...
Apple-framework implementations of the SwiftSTT pipeline. AVFoundation for capture, Accelerate for DSP, Core ML for the encoder and decoder.

## Overview

`SwiftSTTKit` is where the abstract protocols in `SwiftSTTCore` get plugged into Apple frameworks. Everything that needs AVFoundation, Core ML, or Accelerate lives here so that the Core target stays portable and quick to test.
...
- **M7 CLI.** The `swiftstt` executable exposes `transcribe`, `download`, `list-models`, and `info`.
