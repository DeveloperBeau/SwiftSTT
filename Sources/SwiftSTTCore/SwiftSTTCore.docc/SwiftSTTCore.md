...
Pure-logic foundation for SwiftSTT. Models, protocols, and the streaming agreement policy with no Apple framework dependencies.

## Overview

`SwiftSTTCore` is the part of SwiftSTT that does not need AVFoundation, Core ML, or Accelerate. It defines the data types that flow through the pipeline (``AudioChunk``, ``MelSpectrogramResult``, ``WhisperToken``, ``TranscriptionResult``) and the protocols that pipeline stages conform to (``AudioCapturer``, ``VoiceActivityDetector``, ``MelSpectrogramProcessor``, ``Transcriber``).
