...
///
/// The concrete implementation in `AVAudioCapture` (in `SwiftSTTKit`) reads
...
public protocol AudioCapturer: Actor {
    ...
    /// Starts the capture source. Throws ``SwiftSTTError/micPermissionDenied``
    /// if the user has not granted microphone access, or
    /// ``SwiftSTTError/audioCaptureFailed(_:)`` if the underlying engine
    /// could not be started.
    func startCapture() async throws(SwiftSTTError)
