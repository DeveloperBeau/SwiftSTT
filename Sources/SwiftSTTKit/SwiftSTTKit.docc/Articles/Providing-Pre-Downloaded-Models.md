...
1. Download the model once with `swiftstt download base` (or via `ModelDownloader.download(_:)` in a one-off script).
2. Locate the cache directory the CLI used: `~/Library/Caches/SwiftSTT/openai_whisper-base` on macOS, or the iOS Simulator equivalent.
...
func bundledModel() throws(SwiftSTTError) -> ModelBundle {
...
If the loader throws ``SwiftSTTError/modelFileMissing(_:)``, the associated string names which file is missing.
