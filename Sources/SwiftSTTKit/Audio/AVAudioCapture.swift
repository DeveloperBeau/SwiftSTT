...
public actor AVAudioCapture: AudioCapturer {
    ...
    ///
    /// Forwards each delivered buffer onto ``audioStream`` as an ``SwiftSTTCore/AudioChunk``.
    ///
    /// Throws whatever the provider throws. For ``AVMicrophoneInput`` that
    /// includes ``SwiftSTTCore/SwiftSTTError/micPermissionDenied``,
    /// ``SwiftSTTCore/SwiftSTTError/audioConversionFailed``, and
    /// ``SwiftSTTCore/SwiftSTTError/audioCaptureFailed(_:)``.
    ///
    /// Calling more than once without ``stopCapture()`` is a no-op.
    public func startCapture() async throws(SwiftSTTError) {
