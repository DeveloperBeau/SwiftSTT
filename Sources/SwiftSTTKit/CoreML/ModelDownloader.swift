...
public actor ModelDownloader {
    ...
    public static var defaultCacheDirectory: URL {
        ...
        return
            appSupport
            .appendingPathComponent("SwiftSTT", isDirectory: true)
    ...
    ///     the platform default (`~/Library/Application Support/SwiftSTT/Models/`).
    ...
    public func bundle(for model: WhisperModel) throws(SwiftSTTError) -> ModelBundle {
        let dir = cacheDirectory(for: model)
    ...
    /// - Throws: ``SwiftSTTError/modelDownloadFailed(_:)`` on a network,
    ...
    public func downloadCoreMLEncoder(
        ...
        do {
            guard let url = URL(string: urlString) else {
                throw SwiftSTTError.modelDownloadFailed("invalid encoder URL: \(urlString)")
            }
            ...
            if FileManager.default.fileExists(atPath: expected.path) {
                ...
                guard let found = contents.first(where: { $0.pathExtension == "mlmodelc" }) else {
                    throw SwiftSTTError.modelDownloadFailed(
    ...
    public func download(
        _ model: WhisperModel
    ) throws(SwiftSTTError) -> AsyncThrowingStream<DownloadProgress, any Error> {
        ...
        let task = Task { [weak self] in
            do {
                ...
                continuation.finish(
                    throwing: SwiftSTTError.modelDownloadFailed(error.localizedDescription))
            }
    ...
    private func performDownload(
        ...
        let urlString =
            ...
            + model.ggmlFileName
        guard let url = URL(string: urlString) else {
            throw SwiftSTTError.modelDownloadFailed("invalid URL: \(urlString)")
        }
...
private final class DownloadDelegate: NSObject, URLSessionDownloadDelegate, @unchecked Sendable {
    ...
    func urlSession(
        ...
        do {
            ...
            {
                throw SwiftSTTError.modelDownloadFailed("HTTP \(http.statusCode)")
            }
            let fm = FileManager.default
