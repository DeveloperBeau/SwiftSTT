...
struct CommandParsingTests {
    ...
    func rootSubcommands() {
        let names = SwiftWhisper.configuration.subcommands.map { $0._commandName }
    ...
    func helpText() {
        ...
        let rootHelp = SwiftWhisper.helpMessage()
        #expect(rootHelp.contains("swiftwhisper"))
    }
    ...
    static func makeTempFile() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("swiftwhisper-cli-\(UUID().uuidString).wav")
