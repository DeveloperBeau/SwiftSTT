...
struct CacheDirTests {
    ...
    func transcribeAccepts() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("swiftstt-cachedir-\(UUID().uuidString).wav")
    ...
    func tildeExpansion() throws {
        let resolved = try #require(CacheDirectoryOption.resolve("~/swiftstt-cache"))
        let path = resolved.path
        #expect(path.hasPrefix("/"))
        #expect(!path.contains("~"))
        #expect(path.hasSuffix("/swiftstt-cache"))
    }
